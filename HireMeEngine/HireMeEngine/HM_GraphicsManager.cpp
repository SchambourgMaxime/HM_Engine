										   ...
										   glm::vec3 const & position) const
{
	...
	// Check to see if the position is near enough to be seen
	float distance = glm::distance(position, m_pMainCamera->getPosition());

	if (distance > m_frustumFarLimit)
	{

		// if not, check to see if a point of its bounding box is near enough
		std::vector<glm::vec3> boundingBoxVertices = boundingBox.getvertices();

		for (unsigned int i = 0; i < boundingBoxVertices.size(); i++)
		{

			distance = glm::distance(position + boundingBoxVertices[i],
									 m_pMainCamera->getPosition());

			if (distance < m_frustumFarLimit)
				break;
			else if (i == boundingBoxVertices.size() - 1)
				return false;
	...
	if (glm::dot(m_pMainCamera->getOrientation(),
				 position - m_pMainCamera->getPosition()) <= 0.0f)
	{

		// if not, check to see if a point of its bounding box is in front
		std::vector<glm::vec3> boundingBoxVertices = boundingBox.getvertices();
		...
		for (unsigned int i = 0; i < boundingBoxVertices.size(); i++)
		{
			...
			if (glm::dot(m_pMainCamera->getOrientation(),
						 (position + boundingBoxVertices[i]) -
							m_pMainCamera->getPosition()) > 0.0f)
				break;
			else if (i == boundingBoxVertices.size() - 1)
				return false;
