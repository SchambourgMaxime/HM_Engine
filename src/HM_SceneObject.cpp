...
void HM_SceneObject::loadDataMap(HM_FileHandler file)
{
	...
	for (; iter != listStringComponents.end(); iter++)
	{
		...
		m_componentArray.resize(m_componentMap.size());
		unsigned int count = 0;
		for (unsigned int i = 0; i < updateComponentList.size(); i++)
		{
			HM_Component* component = getComponent(updateComponentList[i]);
			if (component)
				m_componentArray[count++] = component;
		}
	}
...
void HM_SceneObject::onUpdateStart()
{
	...
	// Start
	for (unsigned int i = 0; i < m_componentArray.size(); i++)
	{
		HM_Component* currentComponent = m_componentArray[i];
